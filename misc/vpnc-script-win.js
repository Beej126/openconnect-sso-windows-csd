...
case "pre-init":
	break;
case "connect":
	var internal_gw = getInternalGateway();
	...
	run("route add " + env("VPNGATEWAY") + " mask 255.255.255.255 " + getDefaultGateway());
	...
	break;
case "disconnect":
    echo("clearing out routes...");
    // Delete direct route for the VPN gateway to avoid leaving stale route table entries
    run("route -p delete " + env("VPNGATEWAY") + " mask 255.255.255.255", true);
    run("route -p delete 0.0.0.0 mask 0.0.0.0 " + getInternalGateway() + " metric 1", true);
}
